// -----------------------------------------------------------------------------
// Learning focus: Max Min In Array
// Purpose: Demonstrates Max Min In Array and shows the data flow through a complete example.
// Core idea: Follow the helper functions first, then main(), which prepares input and displays the result.
// Complexity note: Complexity depends on the loops and helper functions identified below.
// -----------------------------------------------------------------------------
#include <iostream>
#include <climits>
using namespace std;
// Scans the array while retaining the smallest/largest value seen so far.
int minInArray(int arr[], int size)
{
    int minNum = INT_MAX;
    for (int i = 0; i < size; i++)
    {
        minNum = min(arr[i], minNum);
    }

    return minNum;
}
// Scans the array while retaining the smallest/largest value seen so far.
int maxInArray(int arr[], int size)
{
    int maxNum = INT_MIN;
    for (int i = 0; i < size; i++)
    {
        maxNum = max(arr[i], maxNum);
    }

    return maxNum;
}
int main()
{
    int size;
    cout << "Enter size of array: ";
    cin >> size;
    int numArr[size];
    for (int i = 0; i < size; i++)
    {
        cin >> numArr[i];
    }
    int min = minInArray(numArr, size);
    int max = maxInArray(numArr, size);

    cout << "Max in array is: " << max << endl;
    cout << "Min in array is: " << min << endl;
}
